...
avaliable_bishop_dict_tuple_2 = {
    (1,'b'):frozenset({2,3}),
    (2,'b'):frozenset({3,4}),
    (3,'b'):frozenset({1,4}),
    (4,'b'):frozenset({2,3})
    }
avaliable_knight_dict_tuple_2 = {
    (1,'n'):{2,3,4},
    (2,'n'):{1,3,4},
    (3,'n'):{1,2,4},
    (4,'n'):{1,2,3}}
...
for i in combined_dict.keys():  # i = (1, 2)
    ...
    for j in avaliable_knight_2_dict_tuple.keys():
        ...
        if j in i:
            ...
            combined_dict_2[new_taken] = combined_dict.get(i).intersection(avaliable_knight_2_dict_tuple.get(j))
   # else:
...
combined_dict_emptied = {k: v for k, v in combined_dict_2.items() if v != frozenset()}
print(len(combined_dict_emptied))
print(combined_dict_emptied)